CREATE DEFINER=`root`@`localhost` PROCEDURE `clientesorn`.`sp_saldo`(in productId int,in pwarehouseId int,in pdate datetime,in prawMaterialId int,in returnv int)
begin
