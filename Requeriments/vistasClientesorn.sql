 ...
 create or replace view `clientesorn`.`Maximos` as
 SELECT `StockItemx`.id,max(`StockItemLogx`.id) idx FROM `clientesorn`.`stock_item_logs` AS `StockItemLogx` 
  ...
  create or replace view `clientesorn`.`inventario` as
  ...
  join `clientesorn`.`Maximos` sm on sm.idx=`StockItemLog`.id
...
`Inv`.name,`Inv`.product_type_id,`Inv`.product_id id,`Inv`.packaging_unit, `Inv`.raw_material_id,
  ...
  sum(`Saldo_B`)`Saldo_B`,
  sum(`Saldo_C`)`Saldo_C`
from `clientesorn`.`inventario` as `Inv`
 left join `clientesorn`.`products` AS `Productos` on `Productos`.id=`Inv`.`raw_material_id`
